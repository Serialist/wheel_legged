...
void new_rc_read_data(uint8_t *ReadFromUsart)
{
  ...
  if(ReadFromUsart[SOF_t_0]==First_data&&ReadFromUsart[SOF_t_1]==Second_data)
	{
	 if(Verify_CRC16_Check_Sum(ReadFromUsart,21) == TRUE)
	 { 
		 memcpy(&NEW_rc_data,ReadFromUsart,21);
	 }
	 }
}
	...
	if(ReadFromUsart[SOF_t] == JUDGE_FRAME_HEADER)                   //判断帧头是否为0xa5
	{
		if(Verify_CRC8_Check_Sum(ReadFromUsart,LEN_HEADER )==TRUE)  //帧头CRC校验
		{
			...
			if(Verify_CRC16_Check_Sum(ReadFromUsart,judge_length) == TRUE)//帧尾CRC16校验
			{
				...
				switch(cmdID)
				{
					case ID_keyboard_information:     //0x0304
...
int usart1=0;
uint8_t Usart1_Buffer_Rx[USART1_BUFFER_RX_LEN] = {0};	//锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟捷达拷锟斤拷锟斤拷锟斤拷
...
//		CopeSerial2Data(rec);
		...
		HAL_UART_Receive_DMA(&huart1,Usart1_Buffer_Rx,200);
		//锟斤拷锟斤拷系统锟侥斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
//		rec=USART1_BUFFER_RX_LEN-__HAL_DMA_GET_COUNTER(&hdma_usart1_rx);
		//锟斤拷锟斤拷系统锟侥斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
		  Referee_Read_Data(Usart1_Buffer_Rx);
		
			new_rc_read_data(Usart1_Buffer_Rx);
		
		memset(Usart1_Buffer_Rx, 0, USART1_BUFFER_RX_LEN);	
		// 锟斤拷锟斤拷之锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷
